...
在本项目中进行**任何**开发/维护任务时，默认按 `awesome-code` skill 的规范执行（除非用户明确要求不使用）。

- 先用确定性脚本做任务拆解与路径发现：运行 `scripts/get_path.py` 获取 `agent_coordinator.py` 的真实绝对路径，再调用 `agent_coordinator.py` 输出推荐代理列表与协作策略（single / sequential / parallel）。
- 遇到问题或更优方案时：默认**自行决策**并继续推进，不反问用户；若存在关键假设/取舍，在交付说明中明确写出。
- 交付门槛：修改完成后必须至少通过一次构建验证（`npm run build`）；若改动影响运行时行为（输入/渲染/音效/存档/UI），再补一次本地冒烟运行验证（如 `npm run dev` 或 `npm run preview`，确保控制台无报错且核心流程可玩）。
